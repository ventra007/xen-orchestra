...
describe('dedup tests', () => {
  ...
  it('works in general case ', async () => {
    await Disposable.use(getSyncedHandler({ url: `file://${dir}` }), async handler=>{
        ...
        const value = (await execa('getfattr', ['-n', 'user.hash.sha256', '--only-value', dir+'/in/a/sub/folder/file'])).stdout
        ...
        assert.strictEqual( (await handler.list('xo-block-store/09a3/cd9e/1351/14cb/870a/0b5c/f0df/d3f4/be99/4662/d0c7/15b6/5bcf/c5e3/b635')).length, 0)
